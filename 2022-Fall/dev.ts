...
class Cell {
  ...
  isRecyclable(): boolean {
    const allDestroyed: Cell[] = [this];
    const allTargets: Cell[] = [];
    this.neighbors
      .filter((cell) => cell.finalScrapAmount)
      .forEach((cell) => {
        if (cell.scrapAmount <= this.scrapAmount) {
          allDestroyed.push(cell);
          allTargets.push(...cell.neighbors);
        } else {
          allTargets.push(cell);
        }
      });
    const islands: Island[] = [];
    while (allTargets.length) {
      let currentCells = [allTargets[0]];
      const island = new Island();
      while (currentCells.length) {
        const nextCells: Cell[] = [];
        currentCells.forEach((currentCell) => {
          if (allTargets.includes(currentCell)) {
            allTargets.splice(
              allTargets.findIndex((cell) => cell === currentCell),
              1
            );
          }
          if (allDestroyed.includes(currentCell)) return; // Source cell is the recycler position, ignore it
          if (island.cells.includes(currentCell)) return;
          island.addCell(currentCell);
          nextCells.push(
            ...currentCell.neighbors.filter((cell) => !cell.recycler)
          );
        });
    ...
    // avoid created island without me
    if (
      islands.some(
        (island) => island.owner !== "me" && island.owner !== "contest"
      )
    ) {
      islands.forEach((island) => island.computeFinalSlots());
      const meGarantedPoints = islands.reduce((acc, island) => {
        if (island.owner === "me") return acc + island.finalSlots;
        return acc;
      }, 0);
      const otherPoints = islands.reduce((acc, island) => {
        if (island.owner === "enemy" || island.owner === "contest")
          return acc + island.finalSlots;
        return acc;
      }, 0);
      // Except if it make me win
      console.error(
        `Recycler position ${this.position} generate unreachable cell ${meGarantedPoints} / ${otherPoints}`
      );
      return meGarantedPoints > otherPoints;
    }

    // no created island without me, ok
    return true;
  }
...
class Player {
  ...
  updateIncome(cells: Cell[]) {
    this.income += cells.reduce((acc, cell) => {
      return acc + cell.neighbors.length + 1;
    }, 0);
  }
}
...
class Game {
  ...
  compute() {
    ...
    // Update income of players
    this.me.updateIncome(
      this.cells.filter((cell) => cell.recycler && cell.owner === 1)
    );
    this.enemy.updateIncome(
      this.cells.filter((cell) => cell.recycler && cell.owner === 0)
    );
    ...
    console.error("income", this.me.income, this.enemy.income);
    if (
      this.turn % 1 === 0 &&
      this.me.mater >= 10 &&
      this.me.income < this.enemy.income + 5
    ) {
      ...
      while (preferedRecyclerTargets.length) {
        if (preferedRecyclerTargets[0].isRecyclable()) {
          break; // break while
        }
        preferedRecyclerTargets.shift();
      }
