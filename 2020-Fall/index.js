...
while (true) {
  ...
  spells.sort((a, b) => {
    const apos = a.delta0 > 0 && a.delta1 > 0 && a.delta2 > 0 && a.delta3 > 0;
    const bpos = b.delta0 > 0 && b.delta1 > 0 && b.delta2 > 0 && b.delta3 > 0;
  ...
  for (let i = 1; i >= 0; i--) {
    ...
    if (i === 1) {
      ...
      if (!commande) {
        ...
        const spellIndex = nextSpellToCast(
      ...
      if (commande) {
        console.log(
          `${commande.actionType} ${commande.actionId} ${randomText()}`
        );
...
function howManyTurns(cost, bag, spells) {
  ...
  const leftBag = [...bag];
  const leftSpells = spells.map((spell) => ({ ...spell }));
  let turns = 0;
  turns =
  ...
  for (let i = 0; i < 4; i++) {
    if (need[i] < 0 && spellIndex === -1) {
      ...
      spellIndex = spells.findIndex((spell) => {
        ...
        if (spell[`delta${String(i)}`] <= 0 || spell.castable === false)
          return false;
        if (
      ...
      if (spellIndex === -1 && recursiveIndex !== -1)
        spellIndex = recursiveIndex;
    }
  }
